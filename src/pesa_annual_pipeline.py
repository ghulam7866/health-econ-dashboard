...
--------------------------
An automated pipeline designed for an end-to-end web scraping dashboard.
This script ingestion-cleans the forward-filled PESA quarterly representation,
runs structural time series models to handle regime shifts gracefully, 
and extracts a dynamic 6-year local trend forecast.
"""
...
def fit_ucm_and_forecast(annual: pd.DataFrame, exog_cols: list):
    """
    Fits an Unobserved Components Model (UCM) with a Local Linear Trend
    and Exogenous structural variables to generate a dynamic forecast.
    """
    y = annual[TARGET_COL]
