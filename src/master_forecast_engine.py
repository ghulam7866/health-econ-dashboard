    ...
    for display_name, raw_metric_id in METRIC_NAMES.items():
        ...
        if apply_log_transform:
            ...
            ci = (raw_ci.to_numpy() if hasattr(raw_ci, "to_numpy") else np.asarray(raw_ci)) * scale
                ...
                # ============================================================
        # DAMPEN A&E ATTENDANCES SPIKE - SMOOTH THE TRANSITION
        # ============================================================
        ...
        # ============================================================
        # DAMPEN RTT WAITING LIST INITIAL DROP
        # ============================================================
        if display_name == "RTT waiting list (level)":
            last_hist = y[-1]
            first_fore = mean_fc[0]
        ...
        # ============================================================
        # EMERGENCY OVERRIDE: RTT % within 18 weeks
        # ============================================================
        if display_name == "RTT % within 18 weeks (performance)":
        ...
        if n_clamped > 0 and base_cfg.get("model_type") not in ["ARIMA_mean", "ARIMA_random_walk"]:
            print(
                ...
                f"had a negative raw value/CI bound clamped to 0. The chart may look flat — "
                f"check the underlying model fit/order for this metric."
            )
            print(
                f"   [CLAMP DETAIL] {display_name}: raw mean_fc={mean_fc.tolist()}, raw ci_lower={ci[:,0].tolist()}"
            )
