...
sequence for a SINGLE metric defined in exog_config.py.
...
If no metric is specified, runs for A&E attendances (flow) by default.

Outputs:
  - Full console-equivalent log  -> <REPORT_DIR>/backtest_[metric]_log.txt
  - Summary results              -> <REPORT_DIR>/backtest_[metric]_summary.csv
"""

import sys
import os

# Add the src folder to Python path so exog_config can be imported
...
# Now import exog_config from src folder
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  python backtest_individual_metric.py "A&E attendances (flow)"
  python backtest_individual_metric.py "RTT waiting list (level)"
  python backtest_individual_metric.py "GP total appointments (flow)"
  ...
  - GP telephone appointments (flow)
    ...
    if args.list:
        ...
        for key in METRIC_NAMES.keys():
            print(f"  - {key}")
        print(f"\nTotal: {len(METRIC_NAMES)} metrics")
        return
    ...
    try:
        ...
        # Also print a compact summary at the end
        print("\n\n" + "=" * 90)
