"""
backtest_all_metrics.py
------------------------
Runs the full diagnostic -> backtest -> bias summary -> residual diagnostics
sequence for EVERY metric defined in exog_config.py, in one pass.

Reads production specs from MODEL_CONFIG / EXOG_CONFIG / METRIC_NAMES /
FIT_START_OVERRIDES so there is nothing to hand-edit per metric other than
exog_config.py itself. Candidate alternative specs are generated
automatically from each metric's production spec (trend flip, exog
drop/isolation, seasonal & regular over-differencing checks, AR/MA/ARMA
swaps, white-noise baseline) - the same categories you were testing by hand,
just derived once per metric instead of copy-pasted.

Nothing here writes back into exog_config.py. Per the flag-don't-override
rule, this script only flags; you still edit MODEL_CONFIG / EXOG_CONFIG by
hand after reading the report.

Outputs:
  - Full console-equivalent log  -> <REPORT_DIR>/backtest_all_metrics_log.txt
  - Compact cross-metric summary -> <REPORT_DIR>/backtest_all_metrics_summary.csv
"""
...
warnings.filterwarnings("ignore")  # suppress convergence/non-invertibility noise during the sweep
...
# exog_config.py must be importable - it lives in PROJECT_DIR
sys.path.insert(0, PROJECT_DIR)
import exog_config as ec  # noqa: E402  (import after sys.path edit is intentional)

MODEL_CONFIG = ec.MODEL_CONFIG
EXOG_CONFIG = ec.EXOG_CONFIG
METRIC_NAMES = ec.METRIC_NAMES
FIT_START_OVERRIDES = getattr(ec, "FIT_START_OVERRIDES", {})
...
# ---------------------------------------------------------------------------
# Tee logger - everything printed also lands in the log file
# ---------------------------------------------------------------------------
class Tee:
...
def load_series(metric_key):
    """Loads the series for one metric_key, applying FIT_START_OVERRIDES
    (restricting the fit window) if configured for that metric."""
    df = pd.read_csv(INPUT_FILE)
    df["quarter"] = pd.to_datetime(df["quarter"])
    metric_label = METRIC_NAMES[metric_key]
    sub = df[df["metric"] == metric_label].dropna(subset=["value"]).sort_values("quarter")
    if sub.empty:
        raise ValueError(
            f"No rows found for metric='{metric_label}' (key='{metric_key}') in {INPUT_FILE}. "
            f"Check df['metric'].unique() for a label mismatch."
        )
    if metric_key in FIT_START_OVERRIDES:
        cutoff = pd.to_datetime(FIT_START_OVERRIDES[metric_key])
        before = len(sub)
...
def resolve_horizons_and_min_train(metric_key, sub):
    """Caps horizons per MODEL_CONFIG's 'horizons' key (e.g. GP series
    capped at 3) and shrinks MIN_TRAIN_SIZE for short series so at least a
    handful of backtest folds can actually run. Prints a note whenever it
    adjusts anything, so short-series behaviour is never silent."""
    cfg = MODEL_CONFIG[metric_key]
    ...
    # need at least ~3 folds: n - min_train - max(horizons) >= 3
    while n - min_train - max(horizons) < 3 and min_train > 4:
        min_train -= 1
    if n - min_train - max(horizons) < 3:
        # series is just short (e.g. GP ~11 obs) - trim horizons instead
        horizons = [h for h in horizons if n - min_train - h >= 3]
        if not horizons:
            horizons = [1]  # last resort: at least test 1-step-ahead
...
# ---------------------------------------------------------------------------
# Candidate generation - derives an alternative-spec screen from each
# metric's production spec, instead of a hand-written list per metric.
# ---------------------------------------------------------------------------
def build_candidates(metric_key):
    ...
    trend = cfg["trend"]
    exog_cols = list(EXOG_CONFIG.get(metric_key, []))
    p, d, q = order
    ...
    # --- seasonal (over-)differencing check ---
    if D == 1:
        candidates.append((
    ...
    # --- regular (over-)differencing check ---
    if d == 0:
        candidates.append((
    ...
    # de-dupe (same order/seasonal_order/trend/exog combo can arise more than once)
    seen = set()
    deduped = []
    for c in candidates:
...
# ---------------------------------------------------------------------------
# Diagnostic sequence (significance -> quadratic -> stationarity ->
# order selection -> instability). Screening report only - nothing here
# writes back into candidates.
# ---------------------------------------------------------------------------
...
def diagnostic_order_selection(y, exog, seasonal_period):
    ...
    if not scored:
        print("  No grid combination converged - widen ORDER_GRID_* ranges or check the input series.")
...
def fit_and_forecast(y_scaled, exog, order, seasonal_order, trend, horizon):
    try:
        ...
        if exog is not None:
            # Extrapolate each exog column forward using its last observed
            # step-to-step difference (continues ramps, holds flat steps
            # constant), rather than assuming reversion to 0 out of sample.
            last_val = exog[-1, :]
...
def run_backtest(sub, candidates, horizons, min_train_size):
    ...
    # Pre-extract exog arrays keyed by exog_cols tuple, so candidates that
    # share exog_cols reuse the same array but never see each other's data.
    exog_cache = {}
    for label, order, seasonal_order, trend, exog_cols in candidates:
        key = tuple(exog_cols)
...
def _compute_metrics(rows):
    """Shared metric computation so the main table, bias summary, and
    cross-metric summary CSV can never silently drift apart."""
    if not rows:
        return None
    ...
    for label in matched:
        ...
        for h in bias_horizons:
            if h not in results[label]:
                ...
                continue
            metrics = _compute_metrics(results[label][h])
            if metrics is None:
...
# ---------------------------------------------------------------------------
# Residual diagnostics on the FINAL production spec (full-history fit)
# ---------------------------------------------------------------------------
def residual_diagnostics(sub, metric_key):
    ...
    # --- Ljung-Box: no leftover autocorrelation in residuals ---
    ...
    # --- ARCH LM: residual heteroskedasticity ---
    if n >= 12:
        try:
...
def process_metric(metric_key):
    ...
    print(f"# Series label: {METRIC_NAMES[metric_key]}")
    print("#" * 90)
    ...
    diagnostic_exog_cols = list(EXOG_CONFIG.get(metric_key, []))
    ...
    # Pull production-row metrics at the longest tested horizon for the
    # cross-metric summary CSV.
    prod_label = candidates[0][0]  # PRODUCTION is always first
...
def main():
    ...
    log_path = os.path.join(REPORT_DIR, f"backtest_all_metrics_log_{timestamp}.txt")
    summary_path = os.path.join(REPORT_DIR, f"backtest_all_metrics_summary_{timestamp}.csv")

    sys.stdout = Tee(log_path)

    print(f"Run started: {datetime.now().isoformat()}")
    print(f"Input file: {INPUT_FILE}")
    print(f"Metrics to process ({len(METRIC_NAMES)}): {list(METRIC_NAMES.keys())}")

    summary_rows = []
    for metric_key in METRIC_NAMES.keys():
        try:
            row = process_metric(metric_key)
            summary_rows.append(row)
        except Exception as exc:
            print(f"\n!!! METRIC '{metric_key}' FAILED: {type(exc).__name__}: {exc}")
            traceback.print_exc(file=sys.stdout)
            summary_rows.append({
                "metric_key": metric_key, "n_obs": np.nan, "longest_horizon_tested": np.nan,
                "production_spec": "ERROR", "n_folds": 0, "rmse": np.nan, "mae": np.nan,
                "bias": np.nan, "dir_acc": np.nan, "ci_coverage": np.nan,
                "ljung_box": "ERROR", "jarque_bera": "ERROR",
                "arch_heteroskedasticity": "ERROR", "residual_fit_status": "ERROR",
            })
            continue

    print("\n\n" + "=" * 90)
    print("CROSS-METRIC SUMMARY (production spec, longest horizon tested)")
    print("=" * 90)
    ...
    # Flag rows worth a manual look, without editing anything
    print("\n" + "=" * 90)
    print("QUICK FLAGS (things worth reviewing by hand)")
    print("=" * 90)
    for _, row in summary_df.iterrows():
        flags = []
        if row.get("ljung_box") == "FLAG":
            flags.append("residual autocorrelation")
        if row.get("jarque_bera") == "FLAG":
            flags.append("non-normal residuals")
        if row.get("arch_heteroskedasticity") == "FLAG":
            flags.append("heteroskedastic residuals")
        if row.get("residual_fit_status") not in ("OK",):
            flags.append(f"fit status: {row.get('residual_fit_status')}")
        if isinstance(row.get("ci_coverage"), (int, float)) and not np.isnan(row.get("ci_coverage")) \
                and row.get("ci_coverage") < 0.5:
            flags.append(f"low CI coverage ({row.get('ci_coverage'):.2f})")
        if flags:
            print(f"  {row['metric_key']:<45} -> {', '.join(flags)}")
    print("\n(No flags printed above for a metric means nothing here needs attention -")
    print(" it does not mean the metric is definitely fine, just that these checks passed.)")
